...
def prepData(data,data_type):
  ...
  data_out = []
  columns = ["index","day_nr","item_name","value","unit","pretty_item_name","pretty_val","day_name","dom","month","timestamp"]
  ...
  for item in data:
    ...
    for key in item.keys():
      ...
      row = [index,day_nr,item_name,value,unit,pretty_item_name,pretty_val,day_name,dom,month,timestamp]
      data_out.append( row )
  panda_out = pd.DataFrame(data_out,columns=columns)
  tmp_printer(panda_out)
  return panda_out

import pprint
def tmp_printer(df):
  # assuming your dataframe is called df
  new_df = df.pivot(index='timestamp', columns='pretty_item_name', values=['pretty_val', 'unit'])
  new_df.columns = [f'{col[1]}_{col[0]}' for col in new_df.columns]

# if you want to keep the date column as is
#  new_df = new_df.reset_index()
  new_df.columns = ['date'] + list(new_df.columns[1:])
  pprint.pprint(new_df_pivoted)
